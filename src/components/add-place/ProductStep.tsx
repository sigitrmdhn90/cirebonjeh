...
const emptyProduct = (order: number): SubmissionProductDraft => ({
  ...
  imageFile: null, available: true, bestseller: false, order,
});

function ProductImage({ product }: { product: SubmissionProductDraft }) {
  const src = useMemo(() => product.imageFile ? URL.createObjectURL(product.imageFile) : "", [product.imageFile]);
